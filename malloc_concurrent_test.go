...
// TestConcurrentGrow verifies that concurrent allocations that trigger
// arena growth are handled correctly.
func TestConcurrentGrow(t *testing.T) {
	assert := assert.New(t)

	// Start with a small arena that can grow
	a := NewArena(1024, Backend(SliceBackend))
	initialCap := a.Cap()
	...
	assert.Greater(a.Cap(), initialCap, "arena should have grown")
